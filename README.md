...
<br>
[更新情報]<br>
2026/7/25 ファームウェア更新しました（GPIOドライブ強度2mAへ、コア温度読込、IO06h不具合修正、その他調整）<br>
2026/7/11 ファームウェア更新しました（ファームウェア共通化、PCG定義特殊対応？、その他調整）<br>
-- <br>
<br>
...
ちなみに、[このスピーカー](https://akizukidenshi.com/catalog/g/g112495/)では~~音量が足りませんでした。~~ <br>
実装方法間違ってました（剥き出しで使ってました）、名刺を折って作った簡易的な箱に入れたら十分な音量になりました。<br>
青線を＋に接続しています。安くておすすめです。<br>
<p align="left">
  <img src="/jpg/0309.jpg" width="33%">
  <img src="/jpg/0310.jpg" width="33%">
</p>
...
<br>
2026/8/9追記：やはり数時間使っていると徐々に熱は籠り上がって行きます。<br>

電源に外側から送風してやるのも良いかと思いますが、試しに[ファン](https://akizukidenshi.com/catalog/g/g117509/) を内蔵して見ました。<br>
電源は外部USBアダプタから供給しています。この位置でも10℃弱下がります。<br>
コア温度で回線数コントロールも考えましたが、5Vフルで回しても騒音は気にならない程度なのでやめます。<br>
<p align="left">
  <img src="/jpg/0311.jpg" width="33%">
  <img src="/jpg/0312.jpg" width="33%">
</p>
<br>
...
&ensp;IOアドレス EBh IN  現在クロック(単位MHz)読み出し(値は200MHzで0)<br>
&ensp;IOアドレス ECh IN  拡張機能状態読み出し(後述)<br>
&ensp;IOアドレス EDh IN  コア温度(単位℃,サンプリング1分周期)読み出し<br>
...
### 特殊対応？について<br>
「うる星やつら 台風は楽し」でPCG定義されないのを対応しました。<br>
通常はIOアドレス 02h への出力を 1nh 0nh 順でPCG定義をするようにしていますが、<br>
本プログラムに合わせて出力を 07h 1nh の順でもPCG定義する処理を入れました。<br>
PCG定義の実装仕様に相違があるようなので他にも動かない可能性があります、本物の仕様がわかれば良いのですが…<br>
<br>
...
#### ・CGROMの使用（パターンミスのためD1の実装に工夫が必要、V2.1基板は修正済み）<br>
緊急停止スイッチのショート時に、CGROMを使って文字表示をします。<br>
R10(10kΩ),D1(1N4148など),24ピンICソケット(裏面)を実装し、ICソケットにD2316ECを挿します。<br>
...
&ensp;&ensp;ドライブ強度ooを設定する、デフォルト0(2mA)、=4で4mA、4以外は無視する<br>
&ensp;&ensp;dd,eeは通常0にしてください(後述)<br>
<br>
...
&ensp;・書き込み系はSAVEコマンドとモニタのWコマンドと[PC-8001_SD](https://github.com/yanataka60/PC-8001_SD)にあったsd_testで確認しました。<br>
&ensp;・PC側とのシーケンスが崩れた場合は永久waitになる場合があります。その場合はSTOPキーを押しながらリセットし、BASICから out &h7f,&h8a で初期化してください。<br>
&ensp;&ensp;またマシン語ロード時のみcmtファイルの最後まで到達後のロードは先頭に巻き戻し（cmtファイル自動再オープン）を試みるようにしました。<br>
（シルフィードデモは永久ループします→しばらく放置すると*プロンプトで止まるようですが実害なさそうなので放置します）<br>
<br>
使用するにはWeactRP2350Bのファームウェアを最新版(2026/7/11以降)にしてください。<br>
...
&ensp;&ensp;0000 : 63 6F 6E 66 69 67 cc cc cc rr rr rr vv dd ee oo<br>
<br>
&ensp;&ensp;ddを=1にすると有効になります、本物のPC-8001mk2_SDを実装したまま有効しないでください、最悪壊れます。<br>
<br>
&ensp;picotool.exe load -v -x config.bin -t bin -o 0x102fff00<br>
<br>
&ensp;上記は電源投入時の初期状態となりますが、機能有効無効を以下でも切り替えられます。(保存はされません)<br>
&ensp;&ensp;out 15,&hcd:out 4,&hd:out 4,&hd:out 4,&hd<br>
&ensp;&ensp;?hex$(inp(&hec)) で先頭ビットが1なら有効状態です。<br>
<br>
#### ・64KB EMM & 音源拡張<br>
興味があるなら emmフォルダ内を見てください<br>
